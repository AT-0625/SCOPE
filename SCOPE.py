...
if not ((fname.lower().endswith('.csv')) or (fname.lower().endswith('.fits'))):
  print("ERROR: This program supports only .csv / .fits files !!!")
  print("Your session has been TERMINATED !!!",end='\n\n')
  print('*************************************************************************************************************************************************',end='\n\n')
  exit()
...
print(f"The catalog/table extracted from {fname} is (truncated for visualization convienience): ",end='\n\n')
print(readtable,end='\n\n')
print('*************************************************************************************************************************************************',end='\n\n')
...
def save(newtable):
  ...
  while True:
    ...
    if och=='1':
      ...
      print(f"The updated data has been saved to {fname} !!!",end='\n\n')
      break
    ...
    # If the user wants to save the updated data in a new file
      ...
      print(f"The updated data has been saved to {fname_new} !!!",end='\n\n')
      break

    # If the user does not want the updated data to be saved in file
    elif och=='3':
      print("Are you sure you want to discard changes?",end='\n\n')
      ...
      if check in ['y','Y','yes','YES']:
        ...
        print("The changes have been DISCARDED !!!",end='\n\n')
        break
...
def display(display_string,table_to_display):
  ...
  print("2. Full View        -  Entire table with all rows and columns.",end='\n\n')
  print('*************************************************************************************************************************************************',end='\n\n')
...
# Initializing stars_alt here to ensure it always exists
stars_alt = []
...
################################################################################################################################################################################
      ...
      # If the user goes with the choice of grouping records via attributes
        ...
        grpcol=input("Enter the name of the column according to which the grouping of data is to be performed: ")
...
##################################################################################################################################################################################
      ...
      # If the user goes with the choice of filtering entries
        ...
        while y in ['y','Y','yes','YES']:
          ...
          if col in newreadtable.colnames:
            ...
            # If the data type of the filtering column is neither string nor numeric
              ...
              print("The FILTRATION LOOP will commence once again !!!",end='\n\n')
              continue
        ...
        # If user terminates the filtration loop
          ...
          print("The FILTRATION LOOP has been SUCCEESFULLY TERMINATED !!!")
          print("Now the final filtered table will be displayed on the terminal.........",end='\n\n')
...
###################################################################################################################################################################################
      ...
      # If the user goes with the choice of deriving and appending the data
        ...
        print("1. Parallax            (mas)        -  Requires distance (parsecs).")
        print("2. Absolute Magnitude  (M)          -  Requires apparent magnitude and parallax.")
        print("3. Luminosity          (L / L☉)    -   Requires absolute magnitude.")
        print("4. Distance            (parsecs)    -  Requires parallax (milliarcseconds).")
        print("5. Color Index         (e.g., B-V)  -  Requires magnitudes in both bands.",end='\n\n')
        print('*************************************************************************************************************************************************',end='\n\n')
        ...
        # If the user wants to derive and append parallax values
        if chdq=='1':
          # Asking user for the column consisting of distance values to calculate the absolute magnitude values
          dst=input("Enter the name of the column consisting of the values for distance: ")
          print('\n')

          if dst in newt.colnames and isinstance(newt[dst][0],(float,np.floating)):
            # Asking user for the column name under which the derived values will be stored in
            colname=input("Provide a name for the new column consisting of parallax values: ")
            print('\n')

            newt[colname]=1000/(newt[dst])                                      # Calculating the parallax values for each row and appending it to the table
            dispstr="The revised table consisting of parallax values is: "

          else:
            if dst not in newt.colnames:
              print(f"The column {dst} provided does not exist in the file {fname} !!!",end='\n\n')
            else:
              print(f"The column {dst} provided does not have a numeric data type !!!",end='\n\n')
            continue
        ...
        # If user wants to derive and append absolute magnitude values
          ...
          if (apm in newt.colnames) and (plx in newt.colnames) and (isinstance(newt[apm][0],(float,np.floating))) and (isinstance(newt[plx][0],(float,np.floating))):
            ...
            newt[colname]=newt[apm]+(5*(np.log10(newt[plx])))-10
        ...
        # If user wants to derive and append luminosity values
          ...
          if abm in newt.colnames and isinstance(newt[abm][0],(float,np.floating)):
            ...
            newt[colname]=10**((0.4)*(4.83-newt[abm]))
      ...
      # If the user goes with the choice of appending columns from another table or merging any two tables
        ...
        if not ((f1.lower().endswith('.csv')) or (f1.lower().endswith('.fits'))):
          ...
          try:
            ...
            print(f"The table retrieved from {f1} is (truncated for visualization convienience): ",end='\n\n')
            print(t1,end='\n\n')
            print('*************************************************************************************************************************************************',end='\n\n')
            ...
            if chmg=='1':
              ...
              # If the user does not want to continue with column append operation
                ...
                print("The final updated table with all the appended columns will now be displayed on the terminal...........",end='\n\n')
            ...
            # If the user wants to perform a vertical stack for both the tables
              ...
              # If columns mismatch
                ...
                print("Ensure both tables have the same columns in the same order !!!",end='\n\n')
            ...
            # If the user wants to perform a horizontal stack for both the tables
              ...
              # If the number of rows are different
                ...
                print("ERROR: Cannot perform horizontal merge (hstack) - tables must have the same number of rows !!!",end='\n\n')
            ...
            # If the user wants to perform a join on both the tables
              ...
              if comcol in t1.colnames and comcol in t2.colnames:
                ...
                # If the user goes with an inavlid choice for join type
                  ...
                  print("INVALID choice for join type !!!",end='\n\n')
      ...
      # If the user goes with the choice for column/row deletion
        ...
        # If the user wants to delete row(s)
          ...
          while True:
            ...
            try:
              int_rownum=eval(input("Enter a list of row numbers to delete (e.g., [1, 2, 5]): "))
              print('\n')
            except SyntaxError:
              print("ERROR: INVALID input syntax !!! Please enter a proper list like [1, 2, 5] !!!")
              print('Please try again !!!',end='\n\n')
              continue

            # Ensuring the input is a list
            if type(int_rownum)!=list:
              print("ERROR: The input must be a list of integers like [1, 2, 5] !!!")
              print("Please try again !!!",end='\n\n')
              continue
            else:
              break
          ...
          rownum_unadjusted=list(set_rownum)

          # Validating each row index in the list
          for i in range(len(rownum_unadjusted)):

            # Each element must be an integer and must fall within table bounds
            while (type(rownum_unadjusted[i])!=int) or (not (0<=rownum_unadjusted[i]<len(delt))):
              print('\n')
              print(f"ERROR: INVALID row index '{rownum_unadjusted[i]}' !!! MUST be an integer between 1 and {len(delt)} !!!")
              print("Please try again !!!",end='\n\n')

              # Re-prompts the user for this specific element to ensure it is a valid integer
              rownum_unadjusted[i]=only_num(f"Enter a valid integer row index instead of {rownum_unadjusted[i]}: ",'int')

              # Ensuring that the new row index is not out-of-range
              if (not (0<=rownum_unadjusted[i]<len(delt))):
                continue

          # Convert user row numbers (1-based) to 0-based indexing for NumPy
          rownum=(rownum_unadjusted)-(np.ones(len(rownum_unadjusted),dtype=int))
...
#################################################################################################################################################################################
    ...
    while True:
      try:
        ...
        if not -90<=lat<=90:
          ...
          break
      except ValueError: # If it does not lie in the expected range, the loop is reinstated
        print('\n')
        print(f"Please ENTER CORRECT values !!! The latitude {lat}° provided is OUT of -90° to +90° range !!!")
        print("Please try again !!!",end='\n\n')
        print('*************************************************************************************************************************************************',end='\n\n')
        continue
    ...
    while True:
      try:
        ...
        print('\n')
        print(f"Please ENTER CORRECT values !!! The longitude {lon}° provided is OUT of -180° to +180° range !!!")
        print("Please try again !!!",end='\n\n')
        print('*************************************************************************************************************************************************',end='\n\n')
        continue
    ...
    while True:
      try:
        ...
        print('\n')
        print(f"Please ENTER CORRECT values !!! The elevation {ele} meters provided is less than zero !!! Elevation is ALWAYS measured ABOVE SEA LEVEL !!!")
        print("Please try again !!!",end='\n\n')
        print('*************************************************************************************************************************************************',end='\n\n')
        continue
...
#####################################################################################################################################################################################
    ...
    while True:
      ...
      print("2. Visibility (Check and) Window (Full Night Range).")
...
##################################################################################################################################################################################
      ...
      # If the user goes with the choice of visibility (check and) window
        ...
        print("> If your data uses GALACTIC coordinates, please CONVERT them to ICRS using the Coordinate Conversion Tool BEFORE proceeding.")
        print("> Any unsupported option (not 1 or 2) will redirect you to the planner to allow coordinate conversion.", end='\n\n')
        print('*************************************************************************************************************************************************', end='\n\n')
        ...
        # If the user goes with the choice of checking visibility window
          ...
          # stars_alt is a global list to store the altitude values of each star for every minute of the full night duration (for plotting Altitude vs Time visibility curves)
          stars_alt.clear()
          ...
          if (ra in plantable.colnames) and (dec in plantable.colnames) and (isinstance(plantable[ra][0],(float,np.floating))) and (isinstance(plantable[dec][0],(float,np.floating))):
            ...
            while True:
              try:
                ...
                print("Please enter the date in the correct format !!!",end='\n\n')
                continue
            ...
            # User goes with an invalid choice for twilight type, default -18° is assumed by the program
              ...
              print("INVALID choice for twilight type !!! Defaulting to Astronomical Twilight (-18°).",end='\n\n')
              twilight=-18
            ...
            print(f"The sunset time for your location coordinates (lat: {lat}°, lon: {lon}°, ele: {ele} m) for twilight {twilight}° was found to be: ",sunset)
            print(f"The sunrise time for your location coordinates (lat: {lat}°, lon: {lon}°, ele: {ele} m) for twilight {twilight}° was found to be: ",sunrise,end='\n\n')
            print('*************************************************************************************************************************************************',end='\n\n')

            print("NOTE: ALTITUDE THRESHOLD FOR STARS")
            print('*************************************************************************************************************************************************', end='\n\n')
            print("> This is the MINIMUM altitude (above the horizon) a star must reach to be considered visible.")
            ...
            try:
              ...
              print(f"The altitude threshold entered was not in the valid range of [5°,15°] !!! Defaulting to the general use altitude threshold of 15°.",end='\n\n')
              minalt=15

            print('*************************************************************************************************************************************************', end='\n\n')
            print("Calculating star visibility window... This may take up to 1–2 minutes...",end='\n\n')
            print('*************************************************************************************************************************************************', end='\n\n')
            ...
            ssr=np.full(len(plantable),np.nan,dtype=object)                     # List storing the starting time of the visibility window for all the stars
            sst=np.full(len(plantable),np.nan,dtype=object)                     # List storing the ending time of the visibility window for all the stars
            ...
            for i in range (len(plantable)):
              ...
              for j in range (len(stalt)):
                if stalt[j]>minalt and stsr is None:
                  stsr=stduration[j]
                  ssr[i]=stsr                                                   # Appeding the start time of visibility window of each and every star to ssr
                elif stalt[j]<minalt and stsr is not None and stst is None:
                  stst=stduration[j]
                  sst[i]=stst                                                   # Appeding the end time of visibility window of each and every star to sst
            ...
            plantable[stsrcol]=ssr
            plantable[ststcol]=sst

            display("The updated table consisting of the starting and ending value for the visibility window is: ",plantable)
            print("NOTE:")
            print('*************************************************************************************************************************************************', end='\n\n')
            print("Empty or 'nan' values indicate stars that did not rise above the selected altitude on the given date !!!",end='\n\n')
            print('*************************************************************************************************************************************************', end='\n\n')
            save(plantable)
...
##################################################################################################################################################################################
    ...
    # Creating a function which saves and displays the plot
    def saveshow(num):
      print('\n')
      print('****************************************************************************************************************************************',end='\n\n')
      print("MANDATORY SAVE REQUIRED: Before viewing the plot, you MUST save the file.")
      ...
      print('****************************************************************************************************************************************',end='\n\n')
      # Displaying the plot
      print("Now displaying the plot on terminal....",end='\n\n')
...
##################################################################################################################################################################################
      ...
      if vizch=='1':
        ...
        print("TIP: GENERAL SCATTER PLOT CAN BE USED FOR STAR MAPS")
        print('****************************************************************************************************************************************',end='\n\n')
        print("> To visualize a star map, use Right Ascension (RA) for X-axis and Declination (Dec) for Y-axis.")
        print("> Color encoding is often based on Apparent Magnitude, but other numeric columns may also be used.",end='\n\n')
        print('****************************************************************************************************************************************',end='\n\n')
        ...
        if xax in readtable.colnames and yax in readtable.colnames and isinstance(readtable[xax][0],(float,np.floating)) and isinstance(readtable[yax][0],(float,np.floating)):
          ...
          # saveshow() allows the plot to be dispayed and saved
          saveshow(1)
...
##################################################################################################################################################################################
      ...
      # If the user wants to plot a column histogram for numeric column
        ...
        if xax in readtable.colnames and isinstance(readtable[xax][0],(float,np.floating)):
          ...
          while True:
            bin_input=input("Enter number of bins (positive integer) or type 'auto' (without apostrophe) for automatic binning: ")
...
##################################################################################################################################################################################
      # If the user wants to plot an altitude vs time graph
        ...
        if labelcol in readtable.colnames and isinstance(readtable[labelcol][0],(str,np.str_)):
          ...
          while True:
            starch=input("Type 'all' / 'specific' (without apostrophe) to plot all / specific stars: ")
            print('\n')

            # If the user wants to plot ALL stars
            if starch.strip().lower()=='all':
              nplot='all'
              break

            # If the user wants to plot SPECIFIC stars
            elif starch.strip().lower()=='specific':

              # Loop to get a valid list from the user
              while True:
                try:
                  strrow_uadj=eval(input("Enter the list consisting of row numbers of stars to be plotted , e.g, [1,6,13]: "))
                  print('\n')
                except SyntaxError:
                  print("ERROR: INVALID input syntax !!! Please enter a proper list like [1, 6, 13] !!!")
                  print("Please try again !!!", end='\n\n')
                  continue

                # Ensuring that input is a list
                if type(strrow_uadj)!=list:
                  print("ERROR: The input must be a LIST of integers like [1, 6, 13] !!!")
                  print("Please try again !!!", end='\n\n')
                  continue
                else:
                  break
              ...
              # Removing duplicates
              strrow_set=set(strrow_uadj)
              strrow_unadjusted=list(strrow_set)

              # Validating each input
              for i in range(len(strrow_unadjusted)):

                # Must be integer and within bounds
                while (type(strrow_unadjusted[i])!=int) or (not (1<=strrow_unadjusted[i]<=len(readtable))):
                  print('\n')
                  print(f"ERROR: INVALID row index '{strrow_unadjusted[i]}' !!! MUST be an integer between 1 and {len(readtable)} !!!")
                  print("Please try again !!!", end='\n\n')

                  # Re-prompt this specific element
                  strrow_unadjusted[i]=only_num(f"Enter a valid integer row index instead of {strrow_unadjusted[i]}: ",'int')

                  if not (1<=strrow_unadjusted[i]<=len(readtable)):
                    continue

              # Convert user row numbers (1-based) to 0-based indexing for NumPy
              strrow=(np.array(strrow_unadjusted) - np.ones(len(strrow_unadjusted), dtype=int)).tolist()

              # If all checks are passed then specific plotting is performed
              starch=strrow
              nplot='specific'
              break
            ...
            # Any invalid keyword
              ...
              print("ERROR: INVALID input DATA TYPE !!! Use 'all' or 'specific' !!!")
              print("Please try again !!!", end='\n\n')
              continue
          ...
          if nplot=='all':
            for i in range(len(readtable)):
              plt.plot(stduration.to_datetime(),stars_alt[i],label=readtable[labelcol][i])
...
##################################################################################################################################################################################
  ...
  # If the user goes with the choice of session termination
    ...
    if choice in ['y','Y','yes','YES']:                                                          # If the user wants to continue with the current session
      ...
      print("Thanks for exploring the stars with us! Wishing you clear skies and great observations ahead !!! :)")
